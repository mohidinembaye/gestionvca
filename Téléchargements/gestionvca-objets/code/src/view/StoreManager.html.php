            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 24px;">
                <div class="panel-card" style="padding: 16px; display: flex; align-items: center; justify-content: space-between; border-left: 4px solid var(--success);">
                    <div>
                        ...
                        <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 4px;"><?php echo $statistiques->getCaEncaisse() ?></div>
                    </div>
                ...
                <div class="panel-card" style="padding: 16px; display: flex; align-items: center; justify-content: space-between; border-left: 4px solid var(--danger);">
                    <div>
                        ...
                        <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 4px;"><?php echo $statistiques->getEnCoursClientsTotal() ?></div>
                    </div>
                    <span style="font-size: 24px;">🛑</span>
                </div>
                <div class="panel-card" style="padding: 16px; display: flex; align-items: center; justify-content: space-between; border-left: 4px solid var(--accent);">
                    <div>
                        ...
                        <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 4px;"><?php echo $statistiques->getCommandesEnregistrees() ?></div>
                    </div>
                    <span style="font-size: 24px;">📊</span>
                </div>
            </div>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                <?php foreach ($panier as $ligne): ?>
                                    <tr>
                                        <td style="padding: 8px 0; font-weight:700;"><?= htmlspecialchars($ligne->getProduit()->getNom()) ?></td>
                                        <td style="padding: 8px 0;"><?= $ligne->getQuantite() ?></td>
                                        <td style="padding: 8px 0; font-weight:800; color:var(--accent);"><?= number_format($ligne->getSousTotal(), 0, ',', ' ') ?> F</td>
                                        <td style="padding: 8px 0; text-align:right;">
                                            <form method="POST" action="/pos" style="display:inline; margin:0;">
                                                <input type="hidden" name="action" value="remove_from_cart">
                                                <input type="hidden" name="produit_id" value="<?= $ligne->getProduit()->getId() ?>">
                    ...
                    <table class="debt-table" id="orders-main-table">
                        ...
                        <tbody>
                            <?php foreach ($commandes as $commande): ?>
                                <?php
                                    $badgeClass = match ($commande->getStatut()) {
                                ...
                                ?>
                                <tr>
                                    <td style="font-weight: 700; color: var(--text-muted);">#<?= htmlspecialchars($commande->getNumCmde()) ?></td>
                                    <td style="font-weight: 700;">
                                        <?= htmlspecialchars($commande->getClient()->getPrenom() . ' ' . $commande->getClient()->getNom()) ?>
                                    </td>
                                    <td style="font-weight: 800; color: var(--accent);"><?= number_format($commande->getMontantTotal(), 0, ',', ' ') ?> F</td>
                                    <td>
                                        <span class="badge <?= $badgeClass ?>"><?= htmlspecialchars($commande->getStatut()) ?></span>
                                    </td>
                                    <td>
                                        <button class="btn-quick-action" onclick="toggleDetails('order-details-<?= $commande->getId() ?>')">Lignes</button>
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="5" style="padding: 0; border: none;">
                                        <div class="details-drawer" id="order-details-<?= $commande->getId() ?>">
                                            ...
                                            <table class="debt-table" style="font-size: 11px;">
                                                ...
                                                <tbody>
                                                    <?php foreach ($commande->getLignes() as $ligne): ?>
                                                        <tr>
                                                            <td><?= htmlspecialchars($ligne->getProduit()->getNom()) ?></td>
            ...
            <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 24px;">
                <div class="panel-card" style="padding: 16px; display: flex; align-items: center; justify-content: space-between; border-left: 4px solid var(--danger);">
                    <div>
                        ...
                        <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 4px;"><?= $sta->getDettesActives() ?></div>
                    </div>
                    <span style="font-size: 24px;">💸</span>
                </div>
                <div class="panel-card" style="padding: 16px; display: flex; align-items: center; justify-content: space-between; border-left: 4px solid var(--warning);">
                    <div>
                        ...
                        <div style="font-size: 18px; font-weight: 800; color: white; margin-top: 4px;"><?= $sta->getNbrClientDebiteur() ?></div>
                    </div>
                    <span style="font-size: 24px;">👥</span>
                </div>
                <div class="panel-card" style="padding: 16px; display: flex; align-items: center; justify-content: space-between; border-left: 4px solid var(--success);">
            ...
            <div style="display: block;">
                ...
                <div class="panel-card" style="margin-bottom: 0;">
                    ...
                    <table class="debt-table" id="debts-main-table">
                        ...
                        <tbody>
                           <?php  foreach($detteactives as $detteactive ):?>
                           <tr id="debt-row-<?= $detteactive->getId() ?>" data-client-name="<?= htmlspecialchars(strtolower($detteactive->getClient()->getPrenom() . ' ' . $detteactive->getClient()->getNom())) ?>" style="transition: all 0.2s;">
                                    <td style="font-weight: 700; color: var(--text-muted);">
                                        #DT-<?= $detteactive->getId() ?><span style="font-size: 10px; color: var(--text-muted); display: block; font-weight: normal; margin-top: 2px;"><?= htmlspecialchars($detteactive->getNumCmde()) ?></span>
                                                                            </td>
                                    <td style="font-size: 12px;"><?= htmlspecialchars($detteactive->getDateCommande()) ?></td>
                                    <td style="font-weight: 700;">
                                        <?= htmlspecialchars($detteactive->getClient()->getPrenom() . ' ' . $detteactive->getClient()->getNom()) ?>
                                    </td>
                                    <td style="font-weight: 700; color: var(--text-main);"><?= number_format($detteactive->getMontantTotal(), 0, ',', ' ') ?> F</td>
                                    <td style="font-weight: 700; color: var(--success);"><?= number_format($detteactive->getMontantVerse(), 0, ',', ' ') ?> F</td>
                                    <td style="color: var(--danger); font-weight: 800;"><?= number_format($detteactive->getMontantRestant(), 0, ',', ' ') ?> F</td>
                                    <td>
                                        <span class="badge badge-danger">
<?= htmlspecialchars($detteactive->getStatut()) ?>                                    </td>
                                    <td style="display: flex; gap: 6px;">
                                        <button class="btn-quick-action" onclick="toggleDetails('debt-lines-<?= $detteactive->getId() ?>')">Articles</button>
                                        <button class="btn-quick-action" style="border-color: var(--accent); color: var(--accent);" onclick="toggleDetails('debt-details-<?= $detteactive->getId() ?>')">💳 Paiements</button>
                                                                                    <button class="btn-quick-action" style="border-color: var(--warning); color: var(--warning);" onclick="toggleDetails('debt-repay-drawer-<?= $detteactive->getId() ?>')">Rembourser</button>
                                                                            </td>
                                </tr>
                                <tr>
                                    <td colspan="8" style="padding: 0; border: none;">
                                        <!-- Drawer 1: Payments list -->
                                        <div class="details-drawer" id="debt-details-<?= $detteactive->getId() ?>">
                                        ...
                                        <div class="details-drawer" id="debt-lines-<?= $detteactive->getId() ?>">
                                            ...
                                            <table class="debt-table" style="font-size: 11px;">
                                                ...
                                                <tbody>
          <?php if (!empty($detteactive->getLignes())): ?>
                                <?php foreach ($detteactive->getLignes() as $ligne): ?>
                                    <tr>
                                        <td><?= htmlspecialchars($ligne->getProduit()->getNom()) ?></td>
                                        <td><?= $ligne->getQuantite() ?></td>
                                        <td><?= number_format($ligne->getPrixUnitaire(), 0, ',', ' ') ?> FCFA</td>
                                        <td><?= number_format($ligne->getSousTotal(), 0, ',', ' ') ?> FCFA</td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr>
                            ...
                            <?php endif; ?>
                                        ...
                                        <!-- Drawer 3: Remboursement form -->
                                                                                      <div class="details-drawer" id="debt-repay-drawer-<?= $detteactive->getId() ?>" style="border: 1px solid rgba(45, 212, 191, 0.25); background: linear-gradient(180deg, rgba(11, 15, 25, 0.95) 0%, rgba(11, 15, 25, 0.98) 100%); border-radius: 14px; padding: 18px 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.4); max-width: 850px; margin: 12px 0;">
                                                 ...
                                                 <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; border-bottom: 1px dashed var(--border-color); padding-bottom: 10px;">
                                                     <div style="display: flex; align-items: center; gap: 8px;">
                                                         ...
                                                         <span style="font-weight: 800; font-size: 13px; color: var(--text-main);">
                                                             Nouveau Remboursement — <span style="color: var(--accent);"><?= htmlspecialchars($detteactive->getClient()->getPrenom() . ' ' . $detteactive->getClient()->getNom()) ?></span>
                                                         </span>
                                                     </div>
                                                     <div style="background: rgba(244, 63, 94, 0.12); border: 1px solid rgba(244, 63, 94, 0.3); padding: 4px 12px; border-radius: 20px; font-size: 11px; font-weight: 800; color: var(--danger);">
                                                         Reste dû : <?= number_format($detteactive->getMontantRestant(), 0, ',', ' ') ?> FCFA
                                                     </div>
                                                 </div>
                                                 ...
                                                 <div style="display: flex; gap: 8px; align-items: center; margin-bottom: 16px;">
                                                     ...
                                                     <button type="button" onclick="setRepayAmount(<?= $detteactive->getId() ?>, <?= (int) $detteactive->getMontantRestant() ?>)" style="background: rgba(45, 212, 191, 0.1); border: 1px solid var(--accent); color: var(--accent); font-size: 10px; font-weight: 700; padding: 4px 10px; border-radius: 6px; cursor: pointer;">Tout solder (<?= number_format($detteactive->getMontantRestant(), 0, ',', ' ') ?> F)</button>
                                                     <button type="button" onclick="setRepayAmount(<?= $detteactive->getId() ?>, <?= (int) ($detteactive->getMontantRestant() / 2) ?>)" style="background: rgba(255, 255, 255, 0.04); border: 1px solid var(--border-color); color: var(--text-main); font-size: 10px; font-weight: 700; padding: 4px 10px; border-radius: 6px; cursor: pointer;">50% (<?= number_format($detteactive->getMontantRestant() / 2, 0, ',', ' ') ?> F)</button>
                                                 </div>
                                                 ...
                                                 <form method="GET" action="#" onsubmit="event.preventDefault(); alert('Action enregistrée (mode démonstration HTML/CSS)');" action="" style="display: flex; gap: 16px; align-items: flex-end; flex-wrap: wrap;">
                                                     ...
                                                     <input type="hidden" name="dette_id" value="<?= $detteactive->getId() ?>">
                                                     ...
                                                     <div style="flex: 1; min-width: 200px;">
                                                         ...
                                                         <div style="position: relative;">
                                                             <input type="number" name="montant_verse" id="repay-input-<?= $detteactive->getId() ?>" class="form-control" max="<?= (int) $detteactive->getMontantRestant() ?>" value="<?= (int) $detteactive->getMontantRestant() ?>" min="1" required style="font-size: 13px; font-weight: 700; padding: 10px 12px; background: #0b0f19; border: 1px solid var(--border-color); color: white; width: 100%;">
                                                         </div>
                                                     </div>
